...
export const CreateAcademic = (payload, onSuccess = null) => async (
  dispatch
) => {
  try {
    dispatch(setLoading(true));
    const data = await axios.post(
      `/employees/academic-history/create`,
      payload
    );
    onSuccess && onSuccess();
...
export const UpdateAcademic = (id, payload, onSuccess = null) => async (
  dispatch
) => {
  try {
    dispatch(setLoading(true));
    const data = await axios.patch(
      `/employees/academic-history/update/${id}`,
      payload
    );
    onSuccess && onSuccess();
