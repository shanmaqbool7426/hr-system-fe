...
export default function FeedbackForm({ onClose, object , task }) {
    const { t } = useTranslation()
    const dispatch = useDispatch()
    const formik = useFormik({
        initialValues: {
            name: object?.name || "",
            member: task?.assignedTo ? [task.assignedTo._id] : [],
    ...
    const formElements = [
        ...
        },
        {
            ...
            name: "member",
            label: t('Employee'),
            value: task?.assignedTo ? `${task.assignedTo.firstName} ${task.assignedTo.lastName}` : "",
            readOnly: true, 
        },
        {
            type: "select",
