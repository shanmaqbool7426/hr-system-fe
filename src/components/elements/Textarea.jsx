...
export default function Textarea({ id, label, error, containerClass, className, ...props }) {
    ...
    return (
        <div className={`zt-formGroup ${containerClass}`}>
            ...
            <textarea
                id={id}
                className={`border dark:bg-gray-700 dark:border-gray-700 dark:text-white border-gray-400 text-gray-900 w-full py-3 px-4 resize-none rounded-md ${error ? " !border-themeDanger " : ""} ${className}`}
                {...props}
            />
