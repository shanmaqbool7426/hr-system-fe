...
export default function SearchSelect({ id, type, containerClass, list, value, onChange, label, placeholder, error, ...props }) {
    ...
    const getDisplayValue = (payload) => {
        if (payload) {
            ...
            if (typeof list[index]?.display === 'string')
    ...
    return (
        <div className={`zt-formGroup ${containerClass}`}>
            <Combobox value={selectedItem} onChange={changeHandler}>
                <>
                    {label && <Combobox.Label className="dark:text-themeGrayscale300" htmlFor={id}>
                    ...
                    <div className='relative'>
                        ...
                        {!props?.readOnly && <div className='zt-actions'>
                            {value && <button className="zt-btnClearAll" onClick={clearInput}><CloseCross aria-hidden="true" /></button>}
                            ...
                            <Combobox.Options className="zt-multiSelectList">
                                {filtered?.length > 0 ? (
                                    filtered.map((item, index) => (
                                        <Combobox.Option key={index} value={item.value} className={"hover:text-white"}>
                                            {({ selected, active }) => (<>
                                                <span className={`block truncate ${selected ? 'font-medium' : 'font-normal'}`}>{item.display}</span>
                                                {selected && <span className={`flex items-center pl-3`}>
                                    ...
                                    <p className="select-none px-4 py-2 text-h6 font-semibold mb-0">Nothing found.</p>
                                )}
                            </Combobox.Options>
                        </Transition>
                    </div>
