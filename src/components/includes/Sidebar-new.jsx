...
export default function Sidebar() {
    ...
    const Navigation = {
        ...
        attendance: [
            ...
            { name: t("Remote Work Request"), href: "/attendance/remote-work-requests" },
            ...
            <div className="border-r border-gray-50 flex flex-col py-4">
                ...
                <span className="flex justify-center">
                    <Link href={'/help-desk'} className="btn-helpDesk text-white">
                        <span className='zt-menuIcon text-white'><HelpLine width={28} height={28} /></span>
                    </Link>
                </span>
            </div>
                ...
                <ul className="flex flex-col gap-3">
                    {Navigation[selected].map((item, index) => (
                        <li key={index}>
                            <div className="flex">
                                {item?.innerSubMenu ?
                                    <a className={`zt-sidebar-link ${router.asPath.includes(item.href) ? "zt-active" : ""}`} href="" onClick={(event) => {
                                        event.preventDefault()
                                    ...
                                    <Link className={`zt-sidebar-link ${router.asPath === item.href ? "zt-active" : ""}`} href={item.href}>
                                        {item.name}
                                    </Link>}
                            </div>


                            {item?.innerSubMenu && <ul className={`${subMenu === item.href ? "" : "hidden"} transition-all`}>
                                {item?.innerSubMenu.map((subItem, index2) => (
                                    <li className="relative py-1" key={index2}>
                                        <div className={`absolute left-0  ${index2 === 0 ? 'h-1/2 top-0.5' : 'h-full -top-4'}  border-l border-b w-4`}></div>
                                        <span className="pl-4 zt-sidebar-link">
                                            <Link className={`zt-sidebar-link ${router.asPath === subItem.href ? "zt-active" : ""}`} href={subItem.href}>
                                                {subItem.name}
                                            </Link>
                                        </span>
                                    </li>
