...
import { Attendance, Connect, Dashboard, Employees, HelpLine, Leave, Organization, Payroll, Projects, Recruitment, RemoteWork, Report } from "../svg";
...
export default function Sidebar() {
    ...
    const toggleVisibilityInnerSubMenu = (openSubIndex) => {
        setOpenSubIndex((prevIndex) => (prevIndex === openSubIndex ? null : openSubIndex));
    };
    ...
    const Navigation = [
        {icon: <Dashboard />, name: t("Dashboard"), href: "/dashboard"},
        {
            ...
            subMenu: [
                ...
                { name: t("Employee Resignation"), href: "/employees/resignation-request"},
        ...
        {icon: <RemoteWork />, name: t("Remote Work"), href: "/remote-work" },
        {
            ...
            subMenu: [
                ...
                {name: t("Schedule Interview"), href: "/recruitment/schedule-interviews"},
            ...
            subMenu: [
                ...
                },
                {
                    ...
                    innerSubMenu: [
                        ...
                        {name: t("Station"), href: "/organization/custom-fields/employee-station"},
                        {name: t("Bank"), href: "/organization/custom-fields/employee-bank"},
    ...
    return (
        <aside id="zt-sidebar" className="zt-sidebar">
            {Navigation.map((item, index) => (
                <AnimatePresence key={index}>
                    {item.subMenu && openIndex === index && (
                        ...
                        >
                            <strong className="block py-[6px] px-[12px] text-h4">{item.name}</strong>
                            <ul className="zt-customScrollbar select-none">
                                {item.subMenu.map((subItem, subIndex) => (
                                    <li
                                        key={subIndex}
                                        className={`${subItem.innerSubMenu && 'zt-hasInnerDropDown'}  ${subIndex === openSubIndex ? ' zt-active' : ''}`}
                                    >
                                        ...
                                        {subItem.innerSubMenu && openSubIndex === subIndex && (
                                            ...
                                            >
                                                <ul>
                                                    {subItem.innerSubMenu.map((innerSubMenuItem, innerSubMenuIndex) => (
                                                        <li key={innerSubMenuIndex} className={router.asPath === innerSubMenuItem.href ? "zt-active" : ""}>
                                                            <Link href={innerSubMenuItem.href}
                                                                onClick={() => {
                                                                    toggleVisibility(index)
                                                                }}>
            ...
            <div className="zt-sidebarHolder">
                <nav id="zt-sidebarNav" className="zt-customScrollbar zt-sidebarNav grow">
                    <ul className="mt-6">
                        {Navigation.map((item, index) => (
                            <li key={index} className={`${item.subMenu && 'zt-hasDropDown'} ${router.asPath.includes(item.href) && "zt-active"}`}>
                                {item?.subMenu ?
                                    <a data-title={item.name} href="#" onClick={() => toggleVisibility(index)}>
                ...
                <span className="px-2 flex flex-col">
                    <Link href={'/help-desk'} className="btn-helpDesk text-white">
                        <span className='zt-menuIcon text-white'><HelpLine /></span>
