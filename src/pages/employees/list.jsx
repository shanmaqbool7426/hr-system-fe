...
export default function EmployeesListPage() {
    ...
    let filteredrows = employees_list.filter((item) => {
        let include = true
    ...
    const rows = paginatedData?.map((item) => {
        return {
            firstName: <Link href={`/employees/details/${item._id}`} className="flex gap-2 items-center no-underline">
        ...
        <section className="flex flex-col grow">
            ...
            <div className="flex justify-between items-center pb-6">
                ...
                <div className="flex items-start gap-2">
                    ...
                    <Button className={"btn btn-primary"} onClick={() => setCreate('employee')}>{t("Add new employee")}</Button>
                </div>
            </div>
            ...
            {create === "employee" && <CreateEmployeeForm employee={selected} onClose={() => {
                setCreate("")
                setSelected(null)
            }} />}
        </section>
