...
import SalaryApprovalForm from "@/components/forms/payRoll/salary/approve";
import SalaryChangesForm from "@/components/forms/payRoll/salary/create";
...
export default function SalaryChangesPage() {
    ...
    const [salary, setSalaryChange] = useState(false)
    ...
    const rows = [
        {
            ...
            action: <DropDown icon={<ThreeDotsVertical />}>
                <ul className="zt-themeDropDownList zt-sm gap-4 w-[123px]">
                    <li className="!p-0">
                        <a onClick={() => { setSalaryChange(true) }} className={'flex items-center no-underline gap-2 cursor-pointer font-normal hover:text-themeSuccessDark'}>
                            <span><EyeOn /></span>
                            <span>{t("View")}</span>
                        </a>
                    </li>
    ...
    return (
        <section className="flex flex-col grow">
            ...
            <div className="flex justify-between pb-6">
                <div className="flex flex-col">
                    <h1 className="text-h4 mb-0">{t("Salary Changes Request")}</h1>
                </div>
                <div className="flex items-start gap-2">
                    <Button className={"btn btn-dark-outline"}>{t("Upload Excel")}</Button>
                    <Button onClick={() => { setSalaryChange(true) }} className={"btn btn-primary"}>{t("Apply For Salary Charges")}</Button>
                </div>
            </div>
            ...
            {salary && <SalaryChangesForm onClose={() => setSalaryChange(false)} />}
            {approve && <SalaryApprovalForm onClose={() => setApprove(false)} />}
