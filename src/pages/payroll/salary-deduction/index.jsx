...
import DeductionApprovalForm from "@/components/forms/payRoll/deduction/approve";
import DeductionForm from "@/components/forms/payRoll/deduction/create";
...
export default function SalaryDeductionPage() {
	...
	const headings = [
		...
		{ title: t("Deduction Details"), col: "DeductionDetails", },
		{ title: t("Tax Details"), col: "TaxDetails" }, 
	...
	const rows = [
		{
			...
			TaxDetails: <div className='flex gap-4 justify-center'>
				<div className='flex flex-col items-start gap-1 text-themeGrayscale500'><span>{t('Is Taxable')}</span> <span>{t('Tax %')}</span></div>
				<div className='flex flex-col gap-1 text-themeGrayscale'><span>{t('Yes')}</span> <span>{t('100%')}</span></div>
			</div>,
			DeductionDetails: <div className='flex gap-4 justify-center'>
				<div className='flex flex-col items-start gap-1 text-themeGrayscale500'><span>{t('Title')}</span> <span>{t('Type')}</span></div>
				<div className='flex flex-col gap-1 text-themeGrayscale'><span>{t('Subsidy')}</span> <span>{t('Lunch')}</span></div>
			</div>,
			Amount: '$100',
			...
			action: <DropDown icon={<ThreeDotsVertical />}>
				<ul className="zt-themeDropDownList zt-sm gap-4 w-[123px]">
					<li className="!p-0">
						<a onClick={() => { setDeduction(true) }} className={'flex items-center no-underline gap-2 cursor-pointer font-normal hover:text-themeSuccessDark'}>
							<span><EyeOn /></span>
							<span>{t("View")}</span>
						</a>
					</li>
	...
	return (
		<section className="flex flex-col grow">
			...
			<div className="flex justify-between pb-6">
				<div className="flex flex-col">
					<h1 className="text-h4 mb-0">{t("Deduction Request")}</h1>
					{/* <p className="mb-0">{t("Manage Payroll here...")}</p> */}
				</div>
				<div className="flex items-start gap-2">
					<Button className={"btn btn-dark-outline"}>{t("Upload Excel")}</Button>
					<Button onClick={()=>{setDeduction(true)}} className={"btn btn-primary"}>{t("Apply For Deduction")}</Button>
				</div>
			</div>
			...
			{deduction && <DeductionForm onClose={() => setDeduction(false)} />}
			{approve && <DeductionApprovalForm onClose={() => setApprove(false)} />}
