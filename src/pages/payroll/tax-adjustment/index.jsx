...
import TaxOpeningForm from "@/components/forms/payRoll/taxAdjustment/opening";
import TaxAdjustmentApprovalForm from "@/components/forms/payRoll/taxAdjustment/approve";
...
export default function PayrollPage() {
	...
	const [taxAdjustment, setTaxAdjustement] = useState(false)
	const [openingTax, setOpeningTax] = useState(false)
	...
	const headings = [
		...
		{ title: t("Time Period"), col: "TimePeriod", },
		{ title: t("Type"), col: "Type", },
		{ title: t("Status"), col: "Status" },
		{ title: t("Creation Details"), col: "CreationDetails" },
	...
	const rows = [
		{
			...
			CreationDetails: <div className='flex gap-4 justify-center'>
				<div className='flex flex-col items-start gap-1 text-themeGrayscale500'><span>{t('Created On')}</span> <span>{t('Created By')}</span></div>
				<div className='flex flex-col gap-1 text-themeGrayscale'><span>{t('20 May 2024')}</span> <span>{t('John')}</span></div>
			</div>,
			TimePeriod: <div className='flex gap-4 justify-center'>
				<div className='flex flex-col items-start gap-1 text-themeGrayscale500'><span>{t('Month')}</span> <span>{t('Year')}</span></div>
				<div className='flex flex-col gap-1 text-themeGrayscale'><span>{t('January')}</span> <span>{t('2025')}</span></div>
			</div>,
			Type: 'Tax Adjustment',
	...
	return (
		<section className="flex flex-col grow">
			...
			<div className="flex justify-between pb-6">
				<div className="flex flex-col">
					<h1 className="text-h4 mb-0">{t("Tax Adjustment Request")}</h1>
				</div>
				<div className="flex items-start gap-2">
					<Button onClick={()=>{setTaxAdjustement(true)}} className={"btn btn-dark-outline"}>{t("Apply For Tax Adjustment")}</Button>
					<Button onClick={()=>{setOpeningTax(true)}} className={"btn btn-primary"}>{t("Apply For Opening Tax")}</Button>
				</div>
			</div>
			...
			{taxAdjustment && <TaxAdjustmentForm onClose={()=>setTaxAdjustement(false)}/>}
			{openingTax && <TaxOpeningForm onClose={()=>setOpeningTax(false)}/>}
			{approve && <TaxAdjustmentApprovalForm onClose={()=>setApprove(false)}/>}
		</section>
	)
}