...
export default function AllowancePage() {
	...
	const headings = [
		...
		{ title: t("Allowance Details"), col: "AllowanceDetails", },
	...
	const rows = [
		{
			...
			AllowanceDetails: <div className='flex gap-4 justify-center'>
				<div className='flex flex-col items-start gap-1 text-themeGrayscale500'><span>{t('Title')}</span> <span>{t('Type')}</span></div>
				<div className='flex flex-col gap-1 text-themeGrayscale'><span>{t('Leave Encashment')}</span> <span>{t('Annual Leave')}</span></div>
			</div>,
	...
	return (
		<section className="flex flex-col grow">
			...
			<div className="flex justify-between pb-6">
				<h1 className="text-h4 mb-0">{t("Allowance Request")}</h1>
				<div className="flex items-start gap-2">
					<Button className={"btn btn-dark-outline"}>{t("Upload Excel")}</Button>
					<Button onClick={() => { setcreate(true) }} className={"btn btn-primary"}>{t("Apply For Allowance")}</Button>
				</div>
			</div>
