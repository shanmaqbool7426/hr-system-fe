    ...
    return (
        <section className="flex flex-col grow">
            ...
            <div className="flex justify-between pb-6">
                <h1 className="text-h4 mb-0">{t("Calender")}</h1>
            </div>
            <div className=" zt-card grow">
                <FilterArea title={t("Calender")}
                    elements={filterElements}
                    filters={filters}
                    setFilters={setFilters}
                />
                <h2 className='font-bold text-xl mb-3'>{t("May 2024")}</h2>
                <div className='grid grid-cols-5'>
                    <span className='text-lg font-bold'>{t("Mon")}</span>
                    <span className='text-lg font-bold'>{t("Tue")}</span>
                    <span className='text-lg font-bold'>{t("Wed")}</span>
                    <span className='text-lg font-bold'>{t("Thu")}</span>
                    <span className='text-lg font-bold'>{t("Fri")}</span>
                    {newSchduleData.map((ele, i) => (
                        <div key={i} className={`p-6 flex flex-col justify-between items-start border border-themeGrayscale300`}>
                     </div>
                    ))}
                </div>
