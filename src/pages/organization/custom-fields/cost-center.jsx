...
export default function ManageCostCenterPage () {
  ...
  const rows = customfield_list.filter(item => item.type === 'cost_center')
    .sort((a, b) => {
      if (sortDir === 'asc')
              ...
              <a onClick={() => {
                Toast.confirmDelete(() => {
                  dispatch(DeleteCustomfield(item._id, () => {
                    Toast.success(t("Cost Center deleted successfully"))
                  }))
                }, t)
  ...
  return (
    <>
      <div className="flex justify-between items-center pb-6">
        <div className="">
          <h1 className="text-h4 mb-0">{t("Cost Center")}</h1>
          <p className="mb-0">{t("Manage your Cost Center")}</p>
        </div>
        <div className="flex items-start gap-2">
          <Button className={"btn btn-primary"} onClick={() => setCreate(true)}>{t("Add New Cost Center")}</Button>
        </div>
      </div>
      ...
      <div className="w-full bg-white p-6 rounded-lg grow">
        ...
        <Table
          ...
          page={page}
          setPage={setPage}
          className={'zt-employeeTable zt-costCenterTable'}
        />
      </div>
      {create && <CreateCustomFieldForm
        title={edit ? t('Cost Center') : t('Cost Center')}
        type={'cost_center'}
