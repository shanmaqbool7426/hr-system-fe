...
export default function ManageMinimumWagePage () {
  ...
  const rows = customfield_list.filter(item => item.type === 'minimum_wage')
    .sort((a, b) => {
      if (sortDir === 'asc')
              ...
              <a onClick={() => {
                Toast.confirmDelete(() => {
                  dispatch(DeleteCustomfield(item._id, () => {
                    Toast.success(t("Minimum Wage deleted successfully"))
                  }))
                }, t)
  ...
  return (
    <>
      <div className="flex justify-between items-center pb-6">
        <div className="">
          <h1 className="text-h4 mb-0">{t("Minimum Wages")}</h1>
          <p className="mb-0">{t("Manage your Minimum Wages")}</p>
        </div>
        <div className="flex items-start gap-2">
          <Button className={"btn btn-primary"} onClick={() => setCreate(true)}>{t("Add New Wage")}</Button>
        </div>
      </div>
      ...
      <div className="w-full bg-white p-6 rounded-lg grow">
        ...
        <Table
          ...
          page={page}
          setPage={setPage}
          className={'zt-employeeTable zt-minimumWageTable'}
        />
      </div>
      {create && <CreateCustomFieldForm
        title={t('Minimum Wage')}
        type={'minimum_wage'}
