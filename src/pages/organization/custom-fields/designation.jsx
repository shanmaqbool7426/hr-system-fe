...
export default function ManageEmployeeDesignationPage() {
  ...
  const rows = customfield_list.filter(item => item.type === 'designation')
    .sort((a, b) => {
      if (sortDir === 'asc')
              ...
              <a onClick={() => {
                Toast.confirmDelete(() => {
                  dispatch(DeleteCustomfield(item._id, () => {
                    Toast.success(t("Desigantion deleted successfully"))
                  }))
                }, t)
  ...
  return (
    <>
      <div className="flex justify-between items-center pb-6">
        <div className="">
          <h1 className="text-h4 mb-0">{t("Employee Designation")}</h1>
          <p className="mb-0">{t("Manage your employee designation")}</p>
        </div>
        <div className="flex items-start gap-2">
          <Button className={"btn btn-primary"} onClick={() => setCreate(true)}>{t("Create New Designation")}</Button>
        </div>
      </div>
      ...
      {create && <CreateCustomFieldForm
        title={edit ? t('Edit Designation') : t('Create Designation')}
        type={'designation'}
