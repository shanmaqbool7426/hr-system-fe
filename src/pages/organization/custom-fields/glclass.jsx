...
export default function ManageGLClassPage () {
  ...
  const rows = customfield_list.filter(item => item.type === 'gl_class')
    .sort((a, b) => {
      if (sortDir === 'asc')
              ...
              <a onClick={() => {
                Toast.confirmDelete(() => {
                  dispatch(DeleteCustomfield(item._id, () => {
                    Toast.success(t("G.L Class deleted successfully"))
                  }))
                }, t)
  ...
  return (
    <>
      <div className="flex justify-between items-center pb-6">
        <div className="">
          <h1 className="text-h4 mb-0">{t("G.L Class")}</h1>
          <p className="mb-0">{t("Manage your G.L Class")}</p>
        </div>
        <div className="flex items-start gap-2">
          <Button className={"btn btn-primary"} onClick={() => setCreate(true)}>{t("Add New G.L Class")}</Button>
        </div>
      </div>
      ...
      <div className=" zt-card grow">
        ...
        <Table
          ...
          page={page}
          setPage={setPage}
          className={'zt-employeeTable zt-glClassTable'}
        />
      </div>
