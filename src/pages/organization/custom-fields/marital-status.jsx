...
export default function ManageMaritalTypePage () {
  ...
  const rows = customfield_list.filter(item => item.type === 'marital_status')
    .sort((a, b) => {
      if (sortDir === 'asc')
              ...
              <a onClick={() => {
                Toast.confirmDelete(() => {
                  dispatch(DeleteCustomfield(item._id, () => {
                    Toast.success(t("Marital Type deleted successfully"))
                  }))
                }, t)
  ...
  return (
    <>
      <div className="flex justify-between items-center pb-6">
        <div className="">
          <h1 className="text-h4 mb-0">{t("Marital Types")}</h1>
          <p className="mb-0">{t("Manage your Marital Types")}</p>
        </div>
        <div className="flex items-start gap-2">
          <Button className={"btn btn-primary"} onClick={() => setCreate(true)}>{t("Add New Marital Type")}</Button>
        </div>
      </div>
      ...
      <div className=" zt-card grow">
        ...
        <Table
          ...
          page={page}
          setPage={setPage}
          className={'zt-employeeTable zt-maritalTypesTable'}
        />
      </div>
      {create && <CreateCustomFieldForm
        title={t('Marital Type')}
        type={'marital_status'}
