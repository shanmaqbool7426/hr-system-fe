...
export default function ManageEmployeeOnboardingPage () {
  ...
  const headings = [
    { title: t("Task"), col: "name", sort: true },
    { title: t("Modified On"), col: "updatedAt", sort: true },
    { title: t("Action"), col: "action" },
]

  const rows = customfield_list.filter(item => item.type === 'onboarding_task')
  .sort((a, b) => {
      if (sortDir === 'asc')
  ...
  return (
    <>
      <div className="flex justify-between items-center pb-6">
        <div className="">
          <h1 className="text-h4 mb-0">{t("Employee Onboarding Tasks")}</h1>
          <p className="mb-0">{t("Manage your Employee Onboarding Tasks")}</p>
        </div>
        <div className="flex items-start gap-2">
          <Button className={"btn btn-primary"} onClick={() => setCreate(true)}>{t("Add Employee Onboarding Tasks")}</Button>
        </div>
      </div>
      ...
      <div className=" zt-card grow">
        ...
        <Table
          ...
          page={page}
          setPage={setPage}
          className={'zt-employeeTable zt-EmployeeOnboardingTable'}
        />
      </div>
      {create && <CreateCustomFieldForm
        title={t('Employee Onboarding Tasks')}
