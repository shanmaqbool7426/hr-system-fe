...
export default function ManageCityPage() {
  ...
  const rows = customfield_list.filter(item => item.type === 'city')
    .sort((a, b) => {
      if (sortDir === 'asc')
  ...
  return (
    <>
      <div className="flex justify-between items-center pb-6">
        <div className="">
          <h1 className="text-h4 mb-0">{t("Cities")}</h1>
          <p className="mb-0">{t("Manage your Cities")}</p>
        </div>
        <div className="flex items-start gap-2">
          <Button className={"btn btn-primary"} onClick={() => setCreate(true)}>{t("Add New City")}</Button>
        </div>
      </div>
      ...
      <div className=" zt-card grow">
        ...
        <Table
          ...
          page={page}
          setPage={setPage}
          className={'zt-employeeTable zt-cityTable'}
        />
      </div>
      {create && <CreateCustomFieldForm
        title={edit ? t('City') : t('City')}
        type={'city'}
