...
export default function ManageEmployeeExitTypePage () {
  ...
  const rows = customfield_list.filter(item => item.type === 'exit_type')
    .sort((a, b) => {
      if (sortDir === 'asc')
  ...
  return (
    <>
      <div className="flex justify-between items-center pb-6">
        <div className="">
          <h1 className="text-h4 mb-0">{t("Exit Types")}</h1>
          <p className="mb-0">{t("Manage your Exit Types")}</p>
        </div>
        <div className="flex items-start gap-2">
          <Button className={"btn btn-primary"} onClick={() => setCreate(true)}>{t("Add New Exit Type")}</Button>
        </div>
      </div>
