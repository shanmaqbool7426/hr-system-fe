...
export default function ManageDeductionTitlePage() {
  ...
  const rows = customfield_list.filter(item => item.type === 'deduction')
    .sort((a, b) => {
      if (sortDir === 'asc')
              ...
              <a onClick={() => {
                Toast.confirmDelete(() => {
                  dispatch(DeleteCustomfield(item._id, () => {
                    Toast.success(t("Deduction Title deleted successfully"))
                  }))
                }, t)
  ...
  return (
    <>
      <div className="flex justify-between items-center pb-6">
        <div className="">
          <h1 className="text-h4 mb-0">{t("Deduction Title")}</h1>
          <p className="mb-0">{t("Manage your Deduction Title")}</p>
        </div>
        <div className="flex items-start gap-2">
          <Button className={"btn btn-primary"} onClick={() => setCreate(true)}>{t("Create New Deduction")}</Button>
        </div>
      </div>
      ...
      <div className="w-full bg-white p-6 rounded-lg grow">
        ...
        <Table
          ...
          page={page}
          setPage={setPage}
          className={'zt-employeeTable zt-deductionTitleTable'}
        />
      </div>
      {create && <CreateCustomFieldForm
        title={edit ? t('Deduction') : t('Deduction')}
        type={'deduction'}
