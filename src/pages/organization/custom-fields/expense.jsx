...
export default function ManageExpenseUnitPage () {
  ...
  const rows = customfield_list.filter(item => item.type === 'expense')
    .sort((a, b) => {
      if (sortDir === 'asc')
              ...
              <a onClick={() => {
                Toast.confirmDelete(() => {
                  dispatch(DeleteCustomfield(item._id, () => {
                    Toast.success(t("Expense Unit deleted successfully"))
                  }))
                }, t)
  ...
  return (
    <>
      <div className="flex justify-between items-center pb-6">
        <div className="">
          <h1 className="text-h4 mb-0">{t("Expense Units")}</h1>
          <p className="mb-0">{t("Manage your Expense Units")}</p>
        </div>
        <div className="flex items-start gap-2">
          <Button className={"btn btn-primary"} onClick={() => setCreate(true)}>{t("Add New Expense Unit")}</Button>
        </div>
      </div>
      ...
      <div className=" zt-card grow">
        ...
        <Table
          ...
          page={page}
          setPage={setPage}
          className={'zt-employeeTable zt-expenseUnitTable'}
        />
      </div>
      {create && <CreateCustomFieldForm
        title={t('Expense Unit')}
        type={'expense'}
