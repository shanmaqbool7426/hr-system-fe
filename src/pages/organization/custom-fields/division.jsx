...
export default function ManageDivisionPage () {
  ...
  const rows = customfield_list.filter(item => item.type === 'division')
    .sort((a, b) => {
      if (sortDir === 'asc')
  ...
  return (
    <>
      <div className="flex justify-between items-center pb-6">
        <div className="">
          <h1 className="text-h4 mb-0">{t("Divisions")}</h1>
          <p className="mb-0">{t("Manage your Divisions")}</p>
        </div>
        <div className="flex items-start gap-2">
          <Button className={"btn btn-primary"} onClick={() => setCreate(true)}>{t("Add New Division")}</Button>
        </div>
      </div>
      ...
      <div className="w-full bg-white p-6 rounded-lg grow">
        ...
        <Table
          ...
          page={page}
          setPage={setPage}
          className={'zt-employeeTable zt-divisionTable'}
        />
      </div>
      {create && <CreateCustomFieldForm
        title={edit ? t('Division') : t('Division')}
        type={'division'}
