...
export default function ManageGenderPage () {
  ...
  const rows = customfield_list.filter(item => item.type === 'gender')
    .sort((a, b) => {
      if (sortDir === 'asc')
              ...
              <a onClick={() => {
                Toast.confirmDelete(() => {
                  dispatch(DeleteCustomfield(item._id, () => {
                    Toast.success(t("Gender deleted successfully"))
                  }))
                }, t)
  ...
  return (
    <>
      <div className="flex justify-between items-center pb-6">
        <div className="">
          <h1 className="text-h4 mb-0">{t("Genders")}</h1>
          <p className="mb-0">{t("Manage your Genders")}</p>
        </div>
        <div className="flex items-start gap-2">
          <Button className={"btn btn-primary"} onClick={() => setCreate(true)}>{t("Add New Gender")}</Button>
        </div>
      </div>
      ...
      <div className=" zt-card grow">
        ...
        <Table
          ...
          page={page}
          setPage={setPage}
          className={'zt-employeeTable zt-genderTable'}
        />
      </div>
