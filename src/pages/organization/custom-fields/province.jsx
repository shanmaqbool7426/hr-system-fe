...
export default function ManageProvincePage() {
  ...
  const rows = customfield_list.filter(item => item.type === 'province')
    .sort((a, b) => {
      if (sortDir === 'asc')
              ...
              <a onClick={() => {
                Toast.confirmDelete(() => {
                  dispatch(DeleteCustomfield(item._id, () => {
                    Toast.success(t("Province deleted successfully"))
                  }))
                }, t)
  ...
  return (
    <>
      <div className="flex justify-between items-center pb-6">
        <div className="">
          <h1 className="text-h4 mb-0">{t("Provinces")}</h1>
          <p className="mb-0">{t("Manage your Provinces")}</p>
        </div>
        <div className="flex items-start gap-2">
          <Button className={"btn btn-primary"} onClick={() => setCreate(true)}>{t("Add New Province")}</Button>
        </div>
      </div>
      ...
      <div className="w-full bg-white p-6 rounded-lg grow">
        ...
        <Table
          ...
          page={page}
          setPage={setPage}
          className={'zt-employeeTable zt-provinceTable'}
        />
      </div>
