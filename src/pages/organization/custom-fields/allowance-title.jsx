...
export default function ManageAllowanceTitlePage() {
  ...
  const rows = customfield_list.filter(item => item.type === 'allowance')
    .sort((a, b) => {
      if (sortDir === 'asc')
  ...
  return (
    <>
      <div className="flex justify-between items-center pb-6">
        <div className="">
          <h1 className="text-h4 mb-0">{t("Allowance Title")}</h1>
          <p className="mb-0">{t("Manage your Allowance Title")}</p>
        </div>
        <div className="flex items-start gap-2">
          <Button className={"btn btn-primary"} onClick={() => setCreate(true)}>{t("Create New Allowance")}</Button>
        </div>
      </div>
      ...
      <div className="w-full bg-white p-6 rounded-lg grow">
        ...
        <Table
          ...
          page={page}
          setPage={setPage}
          className={'zt-employeeTable zt-allowanceTitleTable'}
        />
      </div>
