...
export default function ManageAreaPage() {
  ...
  const rows = customfield_list.filter(item => item.type === 'area')
    .sort((a, b) => {
      if (sortDir === 'asc')
    ...
    <>
      <div className="flex justify-between items-center pb-6">
        ...
        <div className="flex items-start gap-2">
          <Button className={"btn btn-primary"} onClick={() => setCreate(true)}>{t("Add New Area")}</Button>
        </div>
      </div>
      ...
      <div className="w-full bg-white p-6 rounded-lg grow">
        ...
        <Table
          ...
          page={page}
          setPage={setPage}
          className={'zt-employeeTable zt-areaTable'}
        />
      </div>
