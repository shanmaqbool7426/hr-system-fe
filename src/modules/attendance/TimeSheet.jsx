...
export const TimeSheet = ({ className }) => {
    const { user, todayAttendance, breakTimeStart, checkOutAttendance, getBreaksByAttendance } = useSelector((state) => state.attendance)
        ...
        <div className={`${className} zt-card grid grid-cols-2 gap-4`}>
            ...
            <div className='bg-themeGrayscale50 rounded-lg px-4 py-3 col-span-2 flex flex-wrap gap-2 justify-between'>
                <div className='flex flex-col gap-1'>
                    ...
                    <span className='font-semibold text-themeGrayscale900'>{checkIntimes ? checkIntimes : t("Not checked in")}</span>
                </div>
                <div className='flex flex-col gap-1'>
            ...
            <div className='col-span-2 h-28 w-28 rounded-full border-5 border-themeGrayscale400 bg-themeGrayscale50 mx-auto flex justify-center items-center'>
                <time className='text-xs font-bold' dateTime={workedHours}>{workedHours}</time>
            </div>
            {!todayAttendance || todayAttendance?.checkOutAt ? (
                <>
                    <Button onClick={handleCheckIn} variant={'primary'} className={'flex w-full items-center whitespace-nowrap'} disabled={todayAttendance ? todayAttendance?.checkOutAt ? true : false : false}>Check In</Button>
                ...
                <>
                    {!onBreak ? (
                        <Button onClick={() => handleTakeBreak(todayAttendance?._id)} variant={'orange'} className={'flex w-full items-center whitespace-nowrap col-span-2 sm:col-span-1'}>Take Break <TakeBreakIcon className={'shrink-0'} /></Button>
                    ) : (
                        <Button onClick={() => handleBreakOff(breakTimeStart?._id)} variant={'orange'} className={'flex w-full items-center whitespace-nowrap col-span-2 sm:col-span-1'}>Break Off <TakeBreakIcon className={'shrink-0'} /></Button>
                    )}
                   
                </>
             )} 
             <Button onClick={() => handleCheckOut(todayAttendance?._id)} variant={'primary'} className={'flex w-full items-center whitespace-nowrap col-span-2 sm:col-span-1'} disabled={todayAttendance ? todayAttendance?.checkOutAt ? true : false : true }>Check Out <CheckOutIcon className={'shrink-0'} /></Button>
            <div className='w-full bg-themeGrayscale50 p-4 rounded-lg flex flex-col text-center gap-1'>
                <span>{t("Break")}</span>
                <time className='font-bold' dateTime={totalBreakTime}>{totalBreakTime}</time>
            </div>
            <div className='w-full bg-themeGrayscale50 p-4 rounded-lg flex flex-col text-center gap-1'>
