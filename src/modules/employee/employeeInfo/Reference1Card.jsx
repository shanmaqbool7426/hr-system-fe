...
export default function Reference1Card() {
    ...
    const formik = useFormik({
        initialValues: {
            fullname: employee_details?.reference1?.fullname || "",
            position: employee_details?.reference1?.position || "",
            contact: employee_details?.reference1?.contact || "",
            cnic: employee_details?.reference1?.cnic || "",
            organization: employee_details?.reference1?.organization || "",
            workContact: employee_details?.reference1?.workContact || "",
        },
        ...
        onSubmit: values => {
            dispatch(UpdateEmployee(employee_details._id, { reference1: { ...values } }, () => {
                setEdit(false)
            ...
            </div>
            {
                !edit && <ul className='zt--employeeCardBody'>
                    <li>
                        ...
                        <strong>{employee_details?.reference1?.fullname || '------'}</strong>
                    </li>
                    <li>
                        <span>{t("Position")}</span>
                        <strong>{employee_details?.reference1?.position || '------'}</strong>
                    </li>
                    <li>
                        <span>{t("Contanct Number")}</span>
                        <strong>{employee_details?.reference1?.contact || '------'}</strong>
                    </li>
                    <li>
                        <span>{t("CNIC")}</span>
                        <strong>{employee_details?.reference1?.cnic || '------'}</strong>
                    </li>
                    <li>
                        <span>{t("Organization")}</span>
                        <strong>{employee_details?.reference1?.organization || '------'}</strong>
                    </li>
                    <li>
                        <span>{t("Work Contact Number")}</span>
                        <strong>{employee_details?.reference1?.workContact || '------'}</strong>
                    </li>
                </ul>
            }
