...
import { CreateOnboardingTask, UpdateOnboardingTask, DeleteOnboardingTask } from '@/store/actions/onboarding-offboarding.actions';
import Toast from '@/util/toast';
import { Edit, ThreeDotsVertical, Trash } from '@/components/svg';

export default function OnboardingTasksList() {
    const { onboarding_tasks } = useSelector((state) => state.onboardingoffboarding)
    ...
    const formik = useFormik({
        ...
        onSubmit: () => {
            if (edit) {
                dispatch(UpdateOnboardingTask(edit._id, formik.values, onCompleted))
            } else {
                dispatch(CreateOnboardingTask(formik.values, onCompleted))
            }
        },
    ...
    const toggleActive = (id, active) => {
        dispatch(UpdateOnboardingTask(id, { active: !active }, () => {
            Toast.success(t("Task updated successfully"))
        }))
    }
    const onDelete = (id) => {
        Toast.confirmDelete(() => dispatch(DeleteOnboardingTask(id, () => {
            Toast.success(t("Task deleted successfully"))
        })), t)
    }
    return <div className='zt-card'>
        <div className="flex justify-between items-center">
            <h4>{t("Onboarding Tasks")}</h4>
